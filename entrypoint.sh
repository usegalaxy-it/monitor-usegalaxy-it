...
run_saber() {
  (
    ...
    cp /output/report.html /shared_data/www/reports/report_${current_date}_${current_time}.html 2>/dev/null || echo "No historical copy created"

    chmod -R 755 /shared_data/www

    echo "Rotating files in /shared_data/www/index"
    find /shared_data/www/reports -name "*.html" -mtime +7 -exec rm {} \;
